...
static func configure_room(room: Node3D, room_index: int) -> void:
	...
	match room_index:
		0:  # Open Arena — Phase 3 #21: 2 Glitch Bugs + 1 Memory Leak.
			# Pre-T21 this room was 3x glitch bug — mechanically a single
		...
		3:  # Corridor Ambush — Phase 3 #21: 2 Rogue Processes + 1 Glitch
			...
			# the corridor read as repetitive after the per-iteration mix
