...
7. [x] Retopo all forms with shared UV layout where possible (build_lod0 utility in epic07_task07_retopo.py joins each phase's collection geometry into Compiler_LOD0_P1/P2/P3 — Decimate COLLAPSE applied conditionally if pre-decimate count exceeds target, P1 lands at 3330 tris/1790 verts (within 8K budget), P2 at 3738 tris/2030 verts, P3 at 4442 tris/2392 verts — boss-spec budget under 11K for the largest phase, all 3 LOD0s share the original P1 base topology so unwraps can be ported)
8. [x] UV unwrap (Smart UV Project at 66deg angle limit + 0.02 island margin + pack_islands at 0.012 margin via epic07_task08_uv_unwrap.py — P1 9990 UVs in [0.009,0.989]x[0.009,0.991], P2 11214 UVs in [0.010,0.990]x[0.010,0.987], P3 13326 UVs in [0.010,0.974]x[0.010,0.990], all clean within [0,1] usable area)
9. [x] Bake high-poly detail to game mesh (epic07_task09_bake.py runs Cycles selected-to-active bake from per-phase Compiler_HP_P1/P2/P3 high-poly sources with 2-level subsurf applied down onto matching LOD0 — 12 textures total saved at 1024x1024 to assets/textures/bosses/compiler_boss_p{1,2,3}_{normal,ao,curvature,cavity}.png — tangent normal at cage_extrusion 0.10 + ray_dist 0.20, AO standard, cavity = AO with tight max_ray 0.025, curvature = pointiness emission bake from HP material with ColorRamp 0.40-0.60, single-slot bake target trick to bypass per-material-slot limitation)
