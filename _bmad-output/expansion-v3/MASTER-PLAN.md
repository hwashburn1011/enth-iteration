...
- [x] Epic V3-R3-23 — Polyhaven pillar wired into BossArena.tscn (re-exported pillar-only GLB w/ embedded castle_brick_07 PBR, instanced 4 copies in BossArena.tscn at the 4 placeholder Pillar1-4 positions, hid the original CSGBox3D placeholders. Headless --import 34 steps clean. Polyhaven photoscanned PBR now renders inside an actual gameplay scene — full CDN→curl→Blender→GLB→Godot pipeline validated.)
- [x] Epic V3-R3-24 — Polyhaven cobblestone floor StandardMaterial3D in BossArena (copied cobblestone_floor_04 PBRs into res://assets/textures/polyhaven/, ran headless import to generate UIDs, wired the floor StandardMaterial3D in BossArena.tscn directly via 3 ext_resource Texture2D refs — albedo + normal w/ scale=1.5 + roughness, uv1_scale 8x8 for tile repetition. Pure Godot wire-up, no Blender intermediate needed.)
- [x] Epic V3-R3-25 — Polyhaven forrest_ground in Town.tscn (downloaded forrest_ground_03 CC0 PBR — note Polyhaven UK spelling 'forrest' not 'forest' — copied into res://assets/textures/polyhaven/, replaced the procedural FastNoiseLite grass in town.gd::_apply_town_ground_texture() with real photoscanned PBR via load() at runtime: diffuse + normal_gl + roughness, triplanar uv1_scale 6x6, normal_scale 1.0. The town now ships real grass/dirt photogrammetry instead of Perlin noise.)
- [x] Epic V3-R3-26 — Polyhaven plaster + stone on Town buildings (downloaded plaster_brick_01 PBR for buildings, reused R3-21's rough_block_wall PBR for boundary stone walls — both copied into res://. Replaced town.gd::_make_plaster_material() and _make_stone_material() — both used to be FastNoiseLite Perlin/cellular noise generators, now load() real photoscanned diffuse+nor_gl+rough at runtime. All Town building+boundary surfaces now ship Polyhaven photogrammetry.)
- [x] Epic V3-R3-27 — Polyhaven roof_09 + weathered_planks in Town (downloaded roof_09 terracotta tile PBR ~13.3 MB, copied weathered_planks PBR into res:// from R3-21. Replaced town.gd::_make_roof_tile_material() and _make_wood_material() — both ~40 lines of FastNoiseLite cellular/Perlin noise → ~17 lines that load() the real photoscanned PNGs. Roofs and props now use real terracotta + plank photogrammetry.)
...
- [x] Epic V3-R4-03 — Hero tree R3 wired into 6 Town placements (replaced tree_01.glb with hero_tree_r3.glb in town.gd::_add_tree. Town has 6 _add_tree calls at -15,3 / 14,6 / -4,14 / 16,-12 / -16,-14 / 6,16 with varying canopy_radius. All six spawns now use the R3 sculpted gnarled trunk + 5 extruded branches + 6 SSS canopy clusters, scaled to match the original canopy_radius/1.3 ratio.)
- [x] Epic V3-R4-04 — Hero sword R3 attached to Player.tscn (added hero_sword_r3.glb as a child of Player.tscn Model node, scaled 0.5 + pitched back 30° + offset to right hip 0.45,0.85,0.15. The Globbler hero now visibly holds the sculpted sword w/ carved fuller + 8 rune insets + extruded crossguard wherever the player goes. First R4 prop attached to a character.)
- [x] Epic V3-R4-05 — R3 dungeon wall in CorridorStraight.tscn (added 2 dungeon_wall_r3.glb instances at z=-10 and z=+10 to replace the WallNorth/WallSouth CSGBox3D placeholders. South wall flipped 180° around Y so the carved face points inward. The 6m sculpted wall w/ brick joints + glowing runes + sconce + cracked top exactly matches the corridor's 6m end-cap dimensions.)
- [x] Epic V3-R4-06 — R3 data monolith in StoryRoom centerpiece (instanced data_monolith_r3.glb at scale 1.2 behind the NPCSpawnPoint at z=-3. Hid the SafeAreaProp1+2 CSGBox3D placeholders. The carved circuit traces + display ports + broken upper corner + cyan Pointiness emission + 4 floating fragment shards directly evoke the GDD's "AI agent inside a crumbling computer simulation" framing — perfect for a story room flagship piece.)
- [x] Epic V3-R4-07 — R3 forge + 3 rock formations as Town props (added forge_anvil_r3.glb at -9,-6 near Building1 — gives the town a smithy. Added 3 rock_formation_r3.glb instances on the perimeter at -18,5 / 18,-3 / -8,18 with varying scale 0.9-1.2 for natural variation. Total 4 new R3 sculpts in Town.)
- [x] Epic V3-R4-08 — R3 town hero building wired into Town.tscn (instanced town_hero_building_r3.glb at -10,0,-8 to replace Building1 CSGBox3D placeholder. The R3-06 GLB carries the single-mesh sculpt w/ 11 carved windows + recessed door + extruded gable roof + chimney + door awning + Z-zoned PBR — every visible building now uses real architecture.)
- [x] Epic V3-R4-09 — R3 dungeon doors at LootRoom entry/exit (added 2 dungeon_door_r3.glb instances at z=±5.5, the south one flipped 180° around Y. Each carries the carved 4-panel door + 16 extruded iron studs + keyhole + frame mesh + 2-bone hinge rig + open animation. LootRoom now has telegraphed boundaries via real iron-stud doors instead of just walls.)
- [x] Epic V3-R4-10 — Energy crystal cluster sculpted (NEW R4 sculpt — first sculpt since R3 closed). Single-mesh icosphere base flattened on bottom into a low rock, 5 crystal shards extruded + tip-pinched into 3-sided pyramids at varying heights/angles. Bevel + cube_project UV + multires 1 + Z-zoned cyan glass shader (transmission 0.45 + IOR 1.55 + coat) + Pointiness-driven emission ridges. Baked DIFFUSE + NORMAL high→low. GLB into res://assets/models/props/energy_crystal_r4.glb. Pairs naturally with the existing energy_crystal placeholder slot.)
- [x] Epic V3-R4-11 — R4 energy crystal wired into dungeon rooms (replaced energy_crystal.glb path with energy_crystal_r4.glb in room_base.gd. Combat + corridor rooms randomly spawn 1-3 R4 crystal clusters at randomized scale 0.6-1.2 and rotation. Every dungeon room with the right room_type now scatters real sculpted faceted crystals with cyan glass + Pointiness emission instead of the v2 placeholder.)
- [x] Epic V3-R4-12 — Hero Shield sculpted (NEW R4 sculpt). Round wooden shield from a low-depth cylinder, slight dome curvature on the front face, central iron boss double-extruded forward + tip-pinched into a dome, 12 decorative rivet bumps extruded around the rim band at angle = i*tau/12. Bevel + cube_project UV + multires 1 + Pointiness wood/iron shader (boss + rivets render as polished iron, wood stays warm grain). Baked albedo + normal high→low. GLB into res://assets/models/props/hero_shield_r4.glb. Pairs naturally with R3-13 hero sword on the player.)
- [x] Epic V3-R4-13 — Hero shield attached to Player.tscn (added hero_shield_r4.glb instance to Player.tscn Model node at left hip -0.45,0.85,0.15 with scale 0.7. The Globbler now visibly carries both R3 sword on right hip + R4 shield on left hip — symmetrical sword+shield loadout. Headless import clean.)
- [x] Epic V3-R4-14 — Health potion bottle sculpted (NEW R4 sculpt). 24-vertex cylinder w/ sculpted Z-zone radius — wide belly + shoulder taper + narrow neck. Carved label panel inset on the front. Separate cork stopper cylinder + liquid mesh inside (red glass shader w/ transmission + emission strength 8.0 for the health glow). Bevel + cube_project UV + multires 1 + glass shader (transmission 0.85 + IOR 1.50 + coat) + warm tinted base color. Bake albedo + normal high→low. GLB w/ all 3 meshes — bottle + cork + liquid. Replaces the prompt cube placeholders.)
- [x] Epic V3-R4-15 — Potion bottle wired as health prompt drop (branched dropped_item.gd to load potion_bottle_r4.glb when the dropped item is a health prompt, energy_crystal_r4.glb when it's a compute prompt, falling back to the existing item_pickup crystal otherwise. Skips the rarity material override on R4 sculpts since they have their own glass + emission shaders. Every health drop in the dungeon now spawns a real potion bottle, every compute drop spawns a faceted crystal cluster.)
- [x] Epic V3-R4-16 — Wall torch sconce sculpted (NEW R4 sculpt). Iron wall sconce: cube base 0.4x0.08x0.6m, 4 carved bolt-head insets at the plate corners, bracket arm double-extruded forward + flared into a bowl. Bevel + cube_project UV + multires 1 + Pointiness iron shader (bolt heads + bracket ridges polish to brighter metal vs darker plate). Baked albedo + normal high→low. Separate emission flame icosphere (strength 80) + Cycles point light (energy 80, warm). GLB w/ both meshes. Repeating dungeon wall lighting prop.)
- [x] Epic V3-R4-17 — R4 wall sconce wired into combat+loot dungeon rooms (added repeating wall_sconce_r4.glb instances along east + west walls of every combat and loot room. 2 sconces per wall (mirrored on Z at ±half_z*0.55), 4 total per room. Each sconce rotated to face into the room and gets an OmniLight3D child for actual shadow casting on top of the embedded GLB emission flame. Every dungeon combat/loot room now has visible torchlight along the side walls.)
- [x] Epic V3-R4-18 — R3 dungeon walls in CorridorTurn (added 2 dungeon_wall_r3.glb instances to CorridorTurn.tscn at WallOuterSouth z=9 and WallOuterNorth z=-6 positions, replacing the CSGBox3D placeholders. South wall flipped 180° around Y so the carved face points inward toward the corridor. CorridorTurn end-cap walls now match CorridorStraight (R4-05) for consistent dungeon architecture.)
- [x] Epic V3-R4-19 — R3 dungeon walls in LootRoom (added 4 dungeon_wall_r3.glb instances to LootRoom.tscn flanking the entry/exit doors at z=±6, x=±3. The 6m wall placeholders were 12m wide so each gets 2 sculpted walls side-by-side. South pair flipped 180° around Y so carved faces point inward. LootRoom now has consistent sculpted brick walls + glowing rune carvings on every cardinal side.)
- [x] Epic V3-R4-20 — R3 dungeon walls in CombatOpenArena (added 10 dungeon_wall_r3.glb instances around the 20x16m arena: 3 each on north + south walls at x=-7,0,7 (covering the 20m span), 2 each on east + west walls at z=-5,1 rotated 90° around Y (transform basis 0,0,-1 / 0,1,0 / 1,0,0 for east, mirrored for west). All carved faces point inward toward the arena center. The combat arena now reads as actual dungeon architecture, not gray boxes.)
- [x] Epic V3-R4-21 — R3 walls + pillars in CombatPillars (16x16m room. Added 8 dungeon_wall_r3 instances — 2 per side mirrored on long axis at ±4. Replaced all 4 CSGBox3D pillar placeholders with boss_pillar_r3.glb instances at scale 0.6, positioned at (-3,-3), (3,-3), (-3,3), (3,3). The arena's defining "pillar chamber" feature now uses real Polyhaven-textured castle_brick sculpted pillars instead of gray cubes.)
- [x] Epic V3-R4-22 — R3 walls in CombatCorridorAmbush (8x24m corridor ambush room. Added 10 dungeon_wall_r3 instances — 1 each at the 8m north + south end caps + 4 each on the long 24m east + west walls at z=-9,-3,3,9. East/west rotated 90° around Y. The full corridor ambush perimeter is now sculpted brick + rune-glow walls.)
- [x] Epic V3-R4-23 — R3 walls in CombatElevated (16x16m elevated chamber with platforms + ramps. Added 8 dungeon_wall_r3 instances around the perimeter, 2 per side at ±4. Same wall pattern as CombatPillars (R4-21). The 4 combat scenes now all use the same sculpted wall language: CombatOpenArena (10), CombatPillars (8), CombatCorridorAmbush (10), CombatElevated (8) = 36 R3 wall instances total across all combat rooms.)
- [x] Epic V3-R4-24 — R3 walls in CorridorWide (16x12m wide passage. Added 8 dungeon_wall_r3 instances around the perimeter — 2 per side. Now all 3 corridor variants (CorridorStraight from R4-05, CorridorTurn from R4-18, CorridorWide from R4-24) use sculpted R3 walls. The AlcoveWall CSGBox3D inside the room is left as CSG since it's a half-wall protrusion.)
- [x] Epic V3-R4-25 — R3 walls in StoryRoom (10x10m. Added 4 dungeon_wall_r3 instances — 1 per side, perfect dimensional fit (room 10m, wall 6m wide centered on each face). North + east unrotated, south flipped 180° on Y, west rotated -90° around Y. Combined w/ R4-06 data monolith centerpiece, StoryRoom is now fully sculpted: 4 carved brick walls + cyan-glow data monolith + Polyhaven R3-29 floor.)
...
- [x] Epic V3-R5-03 — Hero wooden bridge sculpted (NEW R5 sculpt). Single-mesh arched bridge from a subdivided cube (1m x 3m x 0.10m), Z-bend arch via cosine curve along Y axis (peak 0.18m at center), carved plank seams on every other Y-row of the top face, 6 extruded railing posts (3 per side at y=-1.20,0,1.20) each double-extruded up + tip-pinched into a square cap. Bevel + cube_project UV + multires 1 + weathered wood Perlin shader. Baked albedo + normal high→low. GLB ready to wire into Town.tscn replacing the wooden_bridge.glb v2 placeholder.)
- [x] Epic V3-R5-04 — R5 wooden bridge wired into Town.tscn (replaced wooden_bridge.glb path with wooden_bridge_r5.glb in town.gd::_build_town_decorations. Scaled 1.6 to fit the original placeholder footprint at z=-6 on the north path. Town now has a real arched + railed sculpted bridge instead of a stretched cube w/ procedural wood.)
- [x] Epic V3-R5-05 — Hero stone well sculpted (NEW R5 sculpt). Hexagonal cylinder base (6-vertex) for the well wall, carved brick joints via inset_individual on every other side face, hollowed top via 6 center-top faces inset + push 0.30 down (deep well opening). Plus 2 vertical wooden roof posts (0.05x0.05x0.6m), 1 horizontal crossbeam at z=1.55, hanging bucket (12-vertex cylinder w/ wood shader), rope thin cylinder. Stone Pointiness shader baked + normal high→low. GLB w/ 6 meshes — well body + 2 posts + crossbeam + bucket + rope.)
