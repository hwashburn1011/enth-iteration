...
func get_game_user_dir() -> String:
	# Use Godot's built-in path which is already properly sanitized for the OS.
	# On Windows, special characters like ':' in project names are sanitized
	# (e.g., "Enth: Iteration" → "Enth- Iteration") and OS.get_user_data_dir()
	# returns the correct sanitized path.
