...
enum InspectorState { IDLE, CAPTURING_FRAMES, MONITORING, RECORDING, MOVING_TO, WATCHING_SIGNALS }

var _state: InspectorState = InspectorState.IDLE
	...
	match _state:
		InspectorState.IDLE:
			if FileAccess.file_exists(REQUEST_PATH):
				_handle_request()
		InspectorState.CAPTURING_FRAMES:
			_process_capture()
		InspectorState.MONITORING:
			_process_monitor()
		InspectorState.RECORDING:
			if FileAccess.file_exists(REQUEST_PATH):
				_handle_request()
		InspectorState.MOVING_TO:
			_process_move_to(_delta)
		InspectorState.WATCHING_SIGNALS:
...
func _cmd_capture_frames(params: Dictionary) -> void:
	...
	_state = InspectorState.CAPTURING_FRAMES
	_capture_one_frame()
...
func _cmd_monitor_properties(params: Dictionary) -> void:
	...
	_state = InspectorState.MONITORING
	_sample_one_frame()
...
func _cmd_watch_signals(params: Dictionary) -> void:
	...
	_state = InspectorState.WATCHING_SIGNALS
...
func _cmd_move_to(params: Dictionary) -> void:
	...
	_pending_command = false
	_state = InspectorState.MOVING_TO
...
func _cmd_start_recording(_params: Dictionary) -> void:
	...
	_state = InspectorState.RECORDING
...
func _input(event: InputEvent) -> void:
	if _state != InspectorState.RECORDING:
		return
