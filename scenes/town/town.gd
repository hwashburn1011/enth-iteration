...
func _build_d4_bush_cluster(geom: Node) -> void:
	...
	## sphere meshes with leafy emissive material.
	...
	for i in 8:
		...
		bush.mesh = bm
		bush.position = D4_CENTER + Vector3(
...
func _build_d4_mushroom_forest(geom: Node) -> void:
	## Epic-4 T7: 8 glowing mushrooms — stem cylinder + cap sphere with
	## bright bioluminescent emission, varied heights.
...
func _build_d4_vine_canopy(geom: Node) -> void:
	## Epic-4 T8: a large overhead vine canopy — 6 thick green vine
	## cylinders crisscrossing 5m above the central area.
	...
	# 3 long horizontal vines along x axis at varied z
	for i in 3:
		var vine: MeshInstance3D = MeshInstance3D.new()
	...
	# 3 long horizontal vines along z axis at varied x
	for i in 3:
		var vine: MeshInstance3D = MeshInstance3D.new()
		var vm: CylinderMesh = CylinderMesh.new()
		vm.top_radius = 0.15
		...
		vine.material_override = vine_mat
		canopy.add_child(vine)
	# Hanging leaves — 12 small green prisms hanging from the vines
	for i in 12:
...
func _build_d4_botanist_npc() -> void:
	## Epic-4 T9: Botanist NPC examining plants — green coat, glasses,
	...
	for ex: float in [-0.13, 0.13]:
		...
		bot.add_child(glass)
	# Magnifying glass — handle + circle
...
func _build_d4_beehive(geom: Node) -> void:
	## Epic-4 T10: a hanging beehive — 3 stacked rounded boxes hanging from
	## a tree-like post + bee particles swarming around.
	...
	# Hive — 3 stacked sphere segments
	...
	hive_mat.metallic = 0.10
	hive_mat.roughness = 0.55
	for i in 3:
